...
### 5. `example_model_usage.py` - Usage Demonstrations

**Purpose**: Provide comprehensive examples and demonstrations of the modular nowcasting system.

#### Features
- Model factory usage examples
- Configuration management demonstrations
- Training pipeline walkthroughs
- Performance comparison utilities

#### Usage
```bash
python scripts/example_model_usage.py
```

#### Output Example
```
SPIRIT Nowcasting - Model Usage Examples
========================================
Available regression models: ['xgboost', 'randomforest', 'mlp']

Example 1: Using XGBoost (default)
----------------------------------
Model type: xgboost
Model class: XGBoostModel

Example 2: Using Random Forest
------------------------------
Model type: randomforest
Model class: RandomForestModel

Training different models:
--------------------------
To train with XGBoost:
python run_nowcasting.py --model-type xgboost

Configuration file example:
---------------------------
In config/config.yaml, set:
nowcasting:
  model_type: 'xgboost'    # or 'randomforest' or 'mlp'
```

### 6. `test_modular_nowcasting.py` - Validation and Testing

**Purpose**: Comprehensive testing suite for validating the modular implementation.

#### Test Categories
1. **Model Factory Tests**: Validate model creation and interfaces
2. **Configuration Tests**: Test configuration loading and validation
3. **Backward Compatibility**: Ensure existing code continues to work
4. **Integration Tests**: End-to-end pipeline validation

#### Usage
```bash
python scripts/test_modular_nowcasting.py
```
...
def test_model_factory():
    ...
    assert len(models) == 3
    assert all(model in models for model in ['xgboost', 'randomforest', 'mlp'])
    
    # Test model creation
    for model_type in models:
        model = ModelFactory.create_model(model_type)
        assert hasattr(model, 'get_optuna_params')
        assert hasattr(model, 'create_model')
...
```bash
#!/bin/bash
# complete_pipeline.sh - Run entire SPIRIT pipeline
...
echo "Starting SPIRIT Pipeline..."

# Stage 1: Data Processing
echo "Stage 1: Processing data..."
python scripts/run_data_processing.py --config $CONFIG_FILE

# Stage 2: Embedding Generation
echo "Stage 2: Generating embeddings..."
python scripts/run_embeddings.py --config $CONFIG_FILE

# Stage 3: Nowcasting Training
echo "Stage 3: Training nowcasting model..."
python scripts/run_nowcasting.py \
    --config $CONFIG_FILE \
    --model-type $MODEL_TYPE

# Stage 4: Forecasting Training
echo "Stage 4: Training forecasting model..."
python scripts/run_forecasting.py --config $CONFIG_FILE
...
```bash
#!/bin/bash
# train_all_models.sh - Train all nowcasting models in parallel

CONFIG_FILE="config/config.yaml"

# Start all models in background
python scripts/run_nowcasting.py --config $CONFIG_FILE --model-type xgboost &
python scripts/run_nowcasting.py --config $CONFIG_FILE --model-type randomforest &
python scripts/run_nowcasting.py --config $CONFIG_FILE --model-type mlp &
...
def handle_common_errors():
    """Common error handling patterns in scripts."""
    ...
    try:
        ...
        print("Check configuration file paths and data directories")
        sys.exit(1)
        ...
        print("GPU memory error. Try reducing batch size or using CPU")
        sys.exit(1)
...
def setup_logging(log_level='INFO'):
    """Setup comprehensive logging for scripts."""
    
    logging.basicConfig(
        level=getattr(logging, log_level),
        format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
        handlers=[
            logging.FileHandler('spirit_pipeline.log'),
    ...
    def log_resources():
        while training_active:
            ...
            if torch.cuda.is_available():
                ...
                print(f"CPU: {cpu_percent}%, Memory: {memory_percent}%, GPU Memory: {gpu_memory:.1f}GB")
            else:
...
```python
def track_training_progress(config, model_type):
    """Enhanced progress tracking with ETA calculation."""
    
    from tqdm import tqdm
    import time
    
    n_trials = config['nowcasting']['n_trials']
    start_time = time.time()
    
    with tqdm(total=n_trials, desc=f"Training {model_type}") as pbar:
        for trial in range(n_trials):
            # Training logic here
            
            # Update progress
            elapsed = time.time() - start_time
            eta = elapsed * (n_trials - trial - 1) / (trial + 1)
...
For complete usage examples and configuration details, see the main [README.md](../README.md) and source code documentation in [src/README.md](../src/README.md).
